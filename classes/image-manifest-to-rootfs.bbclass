...
do_copy_manifest_to_image() {
    ...
    if [[ "${IMAGE_MANIFEST_ROOTFS_ENABLE}" == "y" ]]; then
        ...
        if [ -f ${IMAGE_MANIFEST} ]; then
            ...
            install -D -m 0644 ${IMAGE_MANIFEST} ${IMAGE_ROOTFS}/${IMAGE_MANIFEST_ROOTFS_LOCATION}
        else
            ...
            if [ ! -z "${IMAGE_MANIFEST_GUESSED}" ] && [ -f "${DEPLOY_DIR_IMAGE_GUESSED}/${IMAGE_MANIFEST_GUESSED}" ]; then
                ...
                install -D -m 0644 ${DEPLOY_DIR_IMAGE_GUESSED}/${IMAGE_MANIFEST_GUESSED} ${IMAGE_ROOTFS}/${IMAGE_MANIFEST_ROOTFS_LOCATION}
            else
